...
# import CodeMIA_reconstruct_v2.enums
import enums
...
def extract_identifier(source, lang='python'):
    identifiers = set()
    def traverse(node):
        if node.type == 'identifier':
            if node.text.decode() != '__STR':
                identifiers.add(node.text.decode())
        else:
            # Filter out strings
            if node.type != 'string':
                for child in node.children:
                    identifiers.update(traverse(child))
        # for child in node.children:
        #     identifiers.update(traverse(child))
        return identifiers
    ...
    # query = LANGUAGE[lang].query(QUERY_IDENTIFIER)
    # assign_idf_node = query.captures(root)
    # for idf_node in assign_idf_node:
    #     idf_node = idf_node[0]
    #     variable = source[idf_node.start_byte:idf_node.end_byte]
    #     variables.append(variable)
    # return list(set(variables))
    # tree = parser.parse(bytes(source, "utf8"))
    # Get identifiers from the syntax tree
    
    # identifiers = set()
    # for node in tree.root_node.children:
    #     if node.type == 'identifier':
    #         identifiers.add(node.text.decode())
        all_identifiers = traverse(tree.root_node)
